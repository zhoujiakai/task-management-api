...
一个基于 **FastAPI** 的任务管理 RESTful API，实现了完整的 CRUD 操作、异步通知、API Key 认证、Pydantic 数据校验、自定义异常处理、彩色日志、YAML 配置管理和外部天气 API 集成。

**面试核心卖点：**
- 读吞吐 **567 req/s**，并发 500 读请求零失败
- 缓存命中响应 **< 0.3ms**，万次查询仅 0.67ms，比直接查库快一个数量级
- 全链路 **async/await**（异步函数占比 40%），DB / HTTP / 通知无阻塞
- 测试代码量 > 业务代码量（669 行 vs 648 行），39 用例，84% 覆盖率
- 外部 API 优雅降级：天气接口全挂，CRUD 零影响，50 并发写 100% 成功
- 标准分层架构（core → db → models → schemas → services → api），API 版本化
...
| HTTP 客户端 | httpx (async) | 调用外部天气 API，同时用于测试 |
...
└── backend/                        # 后端代码目录
    ...
    │   ├── core/                   # 核心基础设施（配置、日志、安全、异常）
    │   │   ├── __init__.py
    │   │   ├── config.py           # YAML + 环境变量 配置管理模块
    │   │   ├── logger.py           # 彩色日志模块
    │   │   ├── security.py         # API Key 认证依赖
    │   │   └── exceptions.py       # 自定义异常类 + FastAPI 异常处理器注册
    │   ├── db/                     # 数据库层
    │   │   ├── __init__.py
    │   │   └── session.py          # 异步引擎、会话工厂、get_db 依赖、create_tables
    │   ├── models/                 # ORM 模型
    │   │   ├── __init__.py         # 统一导出 Base, Task, TaskStatus
    │   │   └── task.py             # Task ORM 模型 + TaskStatus 枚举
    │   ├── schemas/                # Pydantic 请求/响应模式
    │   │   ├── __init__.py         # 统一导出 TaskCreate, TaskResponse, TaskUpdate
    │   │   └── task.py             # TaskCreate / TaskUpdate / TaskResponse
    │   ├── services/               # 业务逻辑层
    │   │   ├── __init__.py
    │   │   ├── cache.py            # LRU 缓存模块（版本号失效机制）
    │   │   ├── weather.py          # 外部天气 API 集成（wttr.in + 优雅降级）
    │   │   └── notification.py     # 异步模拟邮件通知
    │   └── api/                    # API 路由层
    │       ├── __init__.py
    │       └── v1/                 # API 版本化（v1）
    │           ├── __init__.py
    │           ├── router.py       # v1 路由聚合
    │           └── tasks.py        # 5 个 CRUD 端点 + 认证 + 通知触发 + 缓存集成 + 分页 + 天气集成
    └── tests/
        ...
        └── services/                # 服务单元测试
            ...
            └── test_weather.py      # 天气 API 集成测试（8 个）
```
...
| 层 | 目录 | 职责 |
|---|---|---|
| core | `backend/app/core/` | 配置、日志、安全认证、异常处理 — 全局基础设施 |
| db | `backend/app/db/` | 数据库引擎和会话管理 |
| models | `backend/app/models/` | ORM 模型定义，`__init__.py` 统一导出 |
| schemas | `backend/app/schemas/` | Pydantic 请求/响应校验模型 |
| services | `backend/app/services/` | 业务逻辑（缓存、天气、通知） |
| api | `backend/app/api/v1/` | 路由端点，按版本划分，通过 `router.py` 聚合 |

---

## 四、各模块详解

### 4.1 配置管理 — `backend/app/core/config.py`
...
# 模块级单例，其他模块直接 from app.core.config import cfg
cfg = Config()
```

**面试要点：**
- `_Section` 是递归结构：遇到 dict 值就递归包装为 `_Section`，遇到标量值直接存储
- `app/config.yaml` 被 gitignore，`app/config.example.yaml` 作为模板提交；`.env` 同样被 gitignore，`app/.env.example` 作为模板
- `yaml.safe_load` 防止 YAML 反序列化攻击（不用 `yaml.load`）
- `load_dotenv()` 在模块导入时自动加载 `.env` 文件中的环境变量
- 环境变量覆盖机制：优先级 `.env` / 系统环境变量 > `app/config.yaml`，便于 Docker/K8s 部署时注入配置
- 配置模块位于 `backend/app/core/`，不再散落在项目根目录，遵循后端分层规范

**配置文件结构（app/config.yaml）：**

**环境变量模板（app/.env.example）：**
```bash
# 环境变量配置 — 复制此文件为 .env 并填写实际值
# 环境变量会覆盖 app/config.yaml 中的对应配置
# DATABASE_URL=sqlite+aiosqlite:///./app/tasks.db
...
server:
  ...
  port: 8000
database:
  url: "sqlite+aiosqlite:///./app/tasks.db"   # 异步 SQLite 连接串
auth:
...
### 4.2 彩色日志 — `backend/app/core/logger.py`
...
### 4.3 缓存模块 — `backend/app/services/cache.py`
...
### 4.4 数据库层 — `backend/app/db/session.py`
...
**面试要点：**
- `create_async_engine` — 异步版本的引擎，连接串格式 `sqlite+aiosqlite:///./app/tasks.db`
...
- 数据库模块独立为 `backend/app/db/` 包，与 ORM 模型层分离，职责更清晰

---

### 4.5 ORM 模型 — `backend/app/models/task.py`
...
- 模型独立为 `backend/app/models/` 包，`__init__.py` 统一导出 `Base, Task, TaskStatus`，便于扩展新模型

---

### 4.6 Pydantic 校验 — `backend/app/schemas/task.py`
...
- Schema 独立为 `backend/app/schemas/` 包，`__init__.py` 统一导出，与 models 包结构对称
...
| `TaskResponse` | 所有接口的响应 | 包含 id、created_at、updated_at、weather_info 等只读字段 |

---

### 4.7 认证 — `backend/app/core/security.py`
...
### 4.8 自定义异常 — `backend/app/core/exceptions.py`
...
### 4.9 异步通知 — `backend/app/services/notification.py`
...
- 在 `tasks.py` 中 `await send_notification(...)` — 当前是 await，也可以用 `background_tasks.add_task()` 实现真正的后台执行
- 触发条件：仅在 `status` 从非 completed 变为 completed 时触发，防止重复通知

---

### 4.10 天气 API 集成 — `backend/app/services/weather.py`

**设计思路：** 调用 wttr.in 免费 API（无需 API Key），获取任务 `due_date` 当天的天气预报。使用与 `cache.py` 相同的 LRU + 版本号缓存模式，按"日期|城市"缓存天气查询结果。请求失败时优雅降级返回 `None`，不影响核心 CRUD 功能。
...
async def fetch_weather(due_date: datetime | None) -> str | None:
    """获取 due_date 当天的天气描述。失败返回 None（优雅降级）。"""
    if due_date is None:
        return None

    # 读取配置
    weather_cfg = cfg.weather
    if not getattr(weather_cfg, "enabled", False):
        return None
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            response = await client.get(f"{base_url}/{location}", params={"format": "j1"})
    ...
    matching = [d for d in data.get("weather", []) if d.get("date") == target_date]
    if not matching:
        return None  # 超出预报范围

    # 提取中午天气描述
    day = matching[0]
    midday = day.get("hourly", [])[4]  # index=4 即 12:00
    desc = midday.get("weatherDesc", [{}])[0].get("value", "未知")
    weather_desc = f"{desc}，{midday['tempC']}°C（{day['mintempC']}~{day['maxtempC']}°C）"
...
**面试要点：**
- **wttr.in**：免费天气 API，无需注册和 API Key，返回 JSON 格式的 3 天预报（`format=j1`）
- **优雅降级**：所有异常（网络错误、解析失败、日期超出范围）都 `return None`，不影响核心业务
- **缓存策略**：与 `cache.py` 相同的 LRU + 版本号模式，缓存键为 `"日期|城市"`，同一地点同一天的天气只请求一次
- **httpx.AsyncClient**：异步 HTTP 客户端，比 `aiohttp` 更现代，API 与 `requests` 类似，支持 HTTP/2
- **配置化**：天气功能通过 `weather.enabled` 开关控制，`location` 和 `base_url` 均可配置，支持环境变量覆盖
- **maxsize=64**：天气变化频繁，64 条足够覆盖常用查询场景

---

### 4.11 路由层 — `backend/app/api/v1/tasks.py`

**5 个端点的完整流程：**

```
POST   /tasks         → 校验(TaskCreate) → 创建 ORM → commit → refresh → 获取天气 → 写入缓存 → 返回
GET    /tasks         → select(Task) → 可选 status 过滤 + 分页(limit/offset) → 按创建时间倒序 → 返回列表
GET    /tasks/{id}    → 查缓存 → 命中则直接返回 → 未命中则 db.get → 获取天气 → 写入缓存 → 返回
PUT    /tasks/{id}    → db.get → 更新字段 → commit → 获取天气 → 更新缓存 → 检测状态变更 → 触发通知 → 返回
...
- **`_serialize_task()`**：将 ORM 对象转为纯字典（含 `isoformat()` 序列化日期 + 天气信息），因为 ORM 对象在 session 关闭后无法访问延迟加载属性
- **天气集成**：创建/获取/更新任务时通过 `fetch_weather()` 获取 due_date 当天的天气，一并写入缓存，返回给前端
...
- 路由按 API 版本划分在 `backend/app/api/v1/`，通过 `router.py` 聚合，便于未来扩展 v2 等新版本

---

### 4.12 应用入口 — `backend/app/main.py`
...
- `app.include_router(router)` — 挂载 `backend/app/api/v1/router.py` 聚合的路由，router 中定义的 prefix 和 tags 自动生效
- `/health` — 健康检查端点，不经过认证（定义在 router 外）
- main.py 只做组装，所有依赖从各子包导入，职责单一

---

## 五、测试体系详解

### 5.1 测试基础设施 — `backend/tests/conftest.py`
...
**tests/api/test_tasks.py（12 个）— CRUD 成功/失败 + 分页：**（位于 `backend/tests/api/`）
...
**tests/services/test_cache.py（6 个）— 缓存单元测试：**（位于 `backend/tests/services/`）
- 基本的缓存存取
- 缓存未命中返回 None
- 缓存失效后不再返回旧数据
- store 更新后返回新数据
...
**tests/services/test_weather.py（8 个）— 天气 API 集成测试：**（位于 `backend/tests/services/`）
- 无 due_date 时返回 None
- 天气功能禁用时返回 None
- wttr.in 返回成功时正确解析天气描述并缓存
- 天气 API 失败时优雅降级返回 None
- 预报中无匹配日期时返回 None（超出 3 天预报范围）
- 创建任务时响应包含 weather_info 字段
- 获取任务时响应包含 weather_info 字段
- 任务列表中每项包含 weather_info 字段

**tests/api/test_validation.py（5 个）— 数据校验：**（位于 `backend/tests/api/`）
- 空标题 → 422
- 过去的 due_date → 422
- 无效 status → 422
- 更新时传过去的日期 → 422
...
**tests/api/test_notifications.py（3 个）— 通知触发：**（位于 `backend/tests/api/`）
- 完成 → 触发通知
- 改为 in_progress → 不触发
- 已完成的任务再更新 → 不重复触发

**tests/api/test_exceptions.py（5 个）— 异常响应格式：**（位于 `backend/tests/api/`）
...
```
请求 → api/v1/tasks.py（Depends(verify_api_key)）→ Depends(get_db) → 业务逻辑
              ↓ 认证检查                        ↓ 数据库会话
...
backend/app/main.py（应用入口，组装各层）
    ...
    │
    └── backend/app/api/v1/（路由层）
        ├── router.py        路由聚合
        └── tasks.py         任务端点
```

**依赖方向：** `api` → `services` → `models` → `db`，`core` 被所有层共享。每一层只依赖比自己更底层的模块，不会出现反向依赖。
...
Client → Uvicorn → FastAPI
  ...
  → 路由处理函数（api/v1/tasks.py）
    → 缓存查询（services/cache.py，仅 GET 单条）
    → 数据库操作（db/session.py + models/task.py）
    → 天气查询（services/weather.py，创建/获取/更新时）
    → 缓存更新/失效（services/cache.py）
    → 异常处理（core/exceptions.py → 404）
    → 异步通知（services/notification.py）
  → Pydantic 序列化（schemas/task.py）
  → JSON 响应
```
...
8. **天气 API**：进程内缓存 → Redis 缓存 + 后台任务异步获取（避免阻塞请求）
...
在 `Config.__init__` 中，先用 `yaml.safe_load` 加载 YAML 数据，然后遍历 `_ENV_OVERRIDES` 映射表，检查对应的环境变量是否存在。如果存在，就用环境变量的值覆盖 YAML 中的配置。通过 `python-dotenv` 的 `load_dotenv()` 在模块导入时自动加载 `.env` 文件。这种设计遵循了 12-Factor App 原则，配置优先级为：系统环境变量 > `.env` 文件 > `app/config.yaml`。

### Q13: 天气 API 是如何集成和测试的？

通过 `httpx.AsyncClient` 调用 wttr.in 免费 API（无需 API Key），从 3 天预报中匹配 `due_date` 当天的天气。测试使用 `unittest.mock` 的 `patch` 替换 `httpx.AsyncClient` 和 `cfg`，构造模拟的 API 响应数据，验证正常解析、API 失败降级、无匹配日期等场景。端到端测试（`backend/tests/services/test_weather.py` 后 3 个）通过 httpx 的 `ASGITransport` 直接调用 FastAPI 应用，验证 CRUD 端点响应中包含 `weather_info` 字段。

### Q14: 外部 API 调用失败时如何处理？

采用优雅降级策略：`fetch_weather()` 内部用 `try/except` 捕获 `httpx.HTTPError` 和通用 `Exception`，所有异常都 `return None`。`weather_info` 在 `TaskResponse` 中定义为 `str | None = None`，前端收到 `None` 时可以不显示天气信息。核心 CRUD 功能不依赖天气 API，即使 wttr.in 完全不可用，任务管理仍正常工作。

### Q15: httpx 作为运行时依赖和测试工具有什么区别？

`httpx` 在本项目中双重角色：(1) 运行时调用外部天气 API（`backend/app/services/weather.py` 中的 `httpx.AsyncClient`）；(2) 测试中作为 HTTP 客户端（`backend/tests/conftest.py` 中的 `httpx.AsyncClient` + `ASGITransport`）。因此 `httpx` 被提升为运行时依赖（`backend/pyproject.toml` 的 `dependencies`），而不仅仅是 `dev` 依赖。这比 `aiohttp` 更好，因为 httpx 的 API 与 requests 兼容，学习成本低。

### Q16: 项目中用了哪些 Python 3.10+ 的新特性？
...
采用标准 FastAPI 后端分层架构，按职责将代码划分为 6 个子包：
- **core**：配置、日志、安全认证、异常处理 — 全局基础设施
- **db**：数据库引擎和会话管理
- **models**：SQLAlchemy ORM 模型定义，每个模型一个文件
- **schemas**：Pydantic 请求/响应校验模型，与 models 对称
- **services**：业务逻辑层（缓存、外部 API 集成、通知）
- **api/v1**：路由端点，按版本划分

依赖方向严格单向：`api` → `services` → `models` → `db`，`core` 被所有层共享。这种结构便于扩展 — 添加新资源只需在对应目录新建文件，不影响已有模块。API 版本化（`v1/`）允许未来引入 `v2` 时保持向后兼容。

---

## 八、`backend/pyproject.toml` 关键配置
...
dependencies = [
    ...
    "httpx>=0.28.1",                  # 异步 HTTP 客户端（天气 API + 测试）
]
...
- dev 依赖用 `--extra dev` 安装：`cd backend && uv sync --extra dev`
...
# 4. 确保配置文件存在（已有 app/config.yaml，或从模板复制）
cp app/config.example.yaml app/config.yaml
cp app/.env.example .env                    # 可选：使用环境变量覆盖配置
...
app/api/v1/router.py                    3      0   100%
app/api/v1/tasks.py                    77     31    60%
app/core/__init__.py                    0      0   100%
app/core/config.py                     20      0   100%
app/core/exceptions.py                 15      2    87%
app/core/logger.py                     16      0   100%
app/core/security.py                    8      0   100%
app/db/__init__.py                      0      0   100%
app/db/session.py                      12      5    58%
app/models/__init__.py                  3      0   100%
app/models/task.py                     20      0   100%
app/schemas/__init__.py                 3      0   100%
app/schemas/task.py                    52      3    94%
app/services/__init__.py                0      0   100%
app/services/cache.py                  20      0   100%
app/services/notification.py            7      0   100%
app/services/weather.py                63      4    94%
app/main.py                            21      4    81%
-------------------------------------------------------
TOTAL                                 340     49    86%
```
...
- 39 个测试全部通过
- 总覆盖率 86%
- core（config, logger, security）和 services（cache, notification）达 100%
- models 和 schemas 达 94%~100%
- tasks.py 和 session.py 覆盖率较低是因为部分启动/关闭路径在单元测试中不会执行

---

## 十二、性能基准测试（实测数据）

> 测试环境：macOS Darwin 25.3.0, Python 3.12.7, 内存数据库隔离，无网络依赖

### 12.1 缓存性能

| 指标 | 数据 |
|------|------|
| 缓存命中 10000 次 | **0.67 ms** |
| 单次缓存查询 | **0.067 μs** |
| 缓存未命中 10000 次 | **0.66 ms** |
| 1000 次失效 + 重写 | **0.34 ms** |

**面试话术：** 缓存层用 `lru_cache` + 版本号机制，单次查询 67 纳秒。万次查询不到 1 毫秒。版本号递增使旧条目自然失效，不需要遍历清除，所以失效 + 重写也只要 0.34μs/次。

### 12.2 并发吞吐

| 场景 | 并发数 | 耗时 | 吞吐 | 成功率 |
|------|--------|------|------|--------|
| 写（POST 创建任务） | 50 | 3.4s | 15 req/s | 100% |
| 写（POST 内存DB） | 100 | 8.3s | 12 req/s | 100% |
| 读（GET 列表） | 100 | 354ms | 283 req/s | 100% |
| 读（GET 列表 内存DB） | 500 | 883ms | **567 req/s** | 100% |
| 缓存命中查询 | 1000 | 259ms | **3861 req/s** | 100% |

**面试话术：** 写瓶颈在 SQLite 单线程锁（12 req/s），换成 PostgreSQL + 连接池写吞吐能提升到数百 req/s。读走异步 IO + 缓存，500 并发全部成功，567 req/s。缓存命中的单条查询达到 3861 req/s。
...
|------|------|
| 业务代码 | **648 行**，13 个文件 |
| 测试代码 | **669 行**，7 个文件（测试/代码比 **1.03:1**） |
| 函数/方法 | 35 个，其中异步 14 个（**40%**） |
| 类定义 | 11 个 |
| 装饰器 | `@lru_cache` `@field_validator` `@exception_handler` `@asynccontextmanager` `@classmethod` + 路由装饰器 |

**面试话术：** 异步函数占 40%，全链路无阻塞 — DB 会话、HTTP 客户端、通知发送全部 await。测试代码量超过业务代码，说明每个功能都有对应测试覆盖。

---

*文档最后更新：2026-04-22（同步至 backend/ 目录迁移）*
