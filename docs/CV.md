...
**设计思路：** 读取 YAML 文件，将嵌套字典转为支持属性访问的对象（`cfg.database.url` 而非 `cfg["database"]["url"]`）。
...
- `config.yaml` 被 gitignore，`config.example.yaml` 作为模板提交，避免密钥泄露
...
### 4.5 Pydantic 校验 — `app/schemas.py`
...
### 4.9 路由层 — `app/router.py`

**5 个端点的完整流程：**

```
POST   /tasks         → 校验请求体(TaskCreate) → 创建 ORM 对象 → db.add → commit → refresh → 返回
GET    /tasks         → select(Task) → 可选 status 过滤 → 按创建时间倒序 → 返回列表
GET    /tasks/{id}    → db.get(Task, id) → 未找到则抛 TaskNotFoundException → 返回
PUT    /tasks/{id}    → db.get → 记录 old_status → model_dump(exclude_unset) 更新字段 → commit → 检测状态变更 → 触发通知 → 返回
DELETE /tasks/{id}    → db.get → db.delete → commit → 204
```
...
4. **配置**：YAML 文件 → 环境变量（`pydantic-settings` 的 `BaseSettings`）
5. **部署**：Uvicorn + Gunicorn 进程管理，或 Docker 容器化
6. **日志**：控制台 → 文件/ELK/Loki
7. **分页**：列表接口加 `limit`/`offset` 参数

### Q10: 项目中用了哪些 Python 3.10+ 的新特性？
...
- 23 个测试全部通过
- 总覆盖率 78%（超过 70% 要求）
- auth、models、notifications 达到 100%
