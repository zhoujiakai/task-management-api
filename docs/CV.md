...
│   ├── router.py           # 5 个 CRUD 端点 + 认证 + 通知触发 + 缓存集成 + 分页 + 天气集成
...
**test_weather.py（8 个）— 天气 API 集成测试：**
- 无 due_date 时返回 None
- 天气功能禁用时返回 None
...
auth.py（认证）      exceptions.py（异常）
      ↓
cache.py（缓存）  ←→  weather.py（外部 API）  ←→  notifications.py（异步通知）
```
...
`httpx` 在本项目中双重角色：(1) 运行时调用外部天气 API（`app/weather.py` 中的 `httpx.AsyncClient`）；(2) 测试中作为 HTTP 客户端（`tests/conftest.py` 中的 `httpx.AsyncClient` + `ASGITransport`）。因此 `httpx` 被提升为运行时依赖（`pyproject.toml` 的 `dependencies`），而不仅仅是 `dev` 依赖。这比 `aiohttp` 更好，因为 httpx 的 API 与 requests 兼容，学习成本低。
...
app/router.py             77     31    60%
app/schemas.py            52      3    94%
app/weather.py            63      4    94%
------------------------------------------
TOTAL                    295     49    83%
```
...
- 39 个测试全部通过
- 总覆盖率 83%（超过 70% 要求）
- auth、cache、models、notifications 达到 100%，weather 达到 94%
- router 和 database 覆盖率较低是因为部分启动/关闭路径在单元测试中不会执行

---

*文档最后更新：2026-04-15（同步至 commit 8bd07ac）*
