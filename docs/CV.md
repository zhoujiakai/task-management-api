...
| 层 | 目录 | 职责 |
|---|---|---|
| core | `app/core/` | 配置、日志、安全认证、异常处理 — 全局基础设施 |
| db | `app/db/` | 数据库引擎和会话管理 |
| models | `app/models/` | ORM 模型定义，`__init__.py` 统一导出 |
| schemas | `app/schemas/` | Pydantic 请求/响应校验模型 |
| services | `app/services/` | 业务逻辑（缓存、天气、通知） |
| api | `app/api/v1/` | 路由端点，按版本划分，通过 `router.py` 聚合 |
...
- 配置模块位于 `app/core/`，不再散落在项目根目录，遵循后端分层规范
...
### 4.3 缓存模块 — `app/services/cache.py`
...
- 数据库模块独立为 `app/db/` 包，与 ORM 模型层分离，职责更清晰

---

### 4.5 ORM 模型 — `app/models/task.py`
...
- 模型独立为 `app/models/` 包，`__init__.py` 统一导出 `Base, Task, TaskStatus`，便于扩展新模型

---

### 4.6 Pydantic 校验 — `app/schemas/task.py`
...
- Schema 独立为 `app/schemas/` 包，`__init__.py` 统一导出，与 models 包结构对称
...
### 4.8 自定义异常 — `app/core/exceptions.py`
...
### 4.9 异步通知 — `app/services/notification.py`
...
### 4.10 天气 API 集成 — `app/services/weather.py`
...
- 路由按 API 版本划分在 `app/api/v1/`，通过 `router.py` 聚合，便于未来扩展 v2 等新版本

---

### 4.12 应用入口 — `app/main.py`
...
- `app.include_router(router)` — 挂载 `app/api/v1/router.py` 聚合的路由，router 中定义的 prefix 和 tags 自动生效
...
**tests/api/test_tasks.py（12 个）— CRUD 成功/失败 + 分页：**
...
**tests/services/test_cache.py（6 个）— 缓存单元测试：**
- 基本的缓存存取
- 缓存未命中返回 None
- 缓存失效后不再返回旧数据
- store 更新后返回新数据
...
**tests/services/test_weather.py（8 个）— 天气 API 集成测试：**
- 无 due_date 时返回 None
- 天气功能禁用时返回 None
...
**tests/api/test_validation.py（5 个）— 数据校验：**
- 空标题 → 422
- 过去的 due_date → 422
- 无效 status → 422
- 更新时传过去的日期 → 422
...
**tests/api/test_notifications.py（3 个）— 通知触发：**
- 完成 → 触发通知
- 改为 in_progress → 不触发
- 已完成的任务再更新 → 不重复触发

**tests/api/test_exceptions.py（5 个）— 异常响应格式：**
...
通过 `httpx.AsyncClient` 调用 wttr.in 免费 API（无需 API Key），从 3 天预报中匹配 `due_date` 当天的天气。测试使用 `unittest.mock` 的 `patch` 替换 `httpx.AsyncClient` 和 `cfg`，构造模拟的 API 响应数据，验证正常解析、API 失败降级、无匹配日期等场景。端到端测试（`test_weather.py` 后 3 个）通过 httpx 的 `ASGITransport` 直接调用 FastAPI 应用，验证 CRUD 端点响应中包含 `weather_info` 字段。
...
`httpx` 在本项目中双重角色：(1) 运行时调用外部天气 API（`app/services/weather.py` 中的 `httpx.AsyncClient`）；(2) 测试中作为 HTTP 客户端（`tests/conftest.py` 中的 `httpx.AsyncClient` + `ASGITransport`）。因此 `httpx` 被提升为运行时依赖（`pyproject.toml` 的 `dependencies`），而不仅仅是 `dev` 依赖。这比 `aiohttp` 更好，因为 httpx 的 API 与 requests 兼容，学习成本低。
...
# 3. 确保配置文件存在（已有 app/config.yaml，或从模板复制）
...
*文档最后更新：2026-04-15（同步至分层架构重构 + 性能基准测试）*
